...
export abstract class AppServiceBase {
  ...
  getfilterParams(params: any, date?: any, name?: any) {
    ...
    httpParams = httpParams.append('docNo', params?.filterModel?.docNo?.filter || params?.filterModel?.cnic?.filter || params?.filterModel?.accountTitle?.filter ||  '');
    httpParams = httpParams.append('businessPartner', (params?.filterModel?.businessPartnerName?.filter || params?.filterModel?.customerName?.filter || params?.filterModel?.vendorName?.filter || name) || '');
